...
const generateAllCombination = async ({seq, filter, lastIndex, results}) => {
  console.log('generateAllCombination start...', seq.length)
  if (!results) results = [];
  while (seq.length > 6) {
    randomize(seq);
  ...
  console.log('generateAllCombination', {results, seq, lastIndex})
...
const updateScore = (length) => {
  const scoreMap = {};
  for(const v of mark6ResultMap.values().slice(0, length ? parseInt(length, 10) : undefined).reverse()) {
    if (v?.no && v?.sno) {
...
const updateScoreRow = (scoreMap, no, sno) => {
  ...
  if (MATCH_ONE) generateScoreCombination(scoreMap, 1, [], options, 1, updateScoreOne);
...
const showTrend = (length) => {
  ...
  Object.entries(sortedScoreMap).filter(e => e[0] !== 'matches').slice(1, 20).forEach(e => {
    console.log(e)
  })
  for (const attr of attrs) {
    const lst = scoreList.map(s => s[attr]).filter(s => !isNaN(s));
    const avg = getMean(lst);
    const std = getStandardDeviation(lst);
    mark6TrendMap.set(attr, {avg,std});
    console.log('summary', {attr, avg, std})
  }
}
...
if (args.length < 3) {
  ...
  if (p1 === 'generate') {
    ...
    generateMark6PossibleResults().then(() => {
      finished = true;
